...
if __name__ == '__main__':
    ...
    if Optimization_case == "Unc_and_base":
        ...
        # Save farm report with designated name and path (this case  HPC)
        report_farm_without_unc = str(kf['p_name'].iloc[0]) + "_report_without_unc.png"
        plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/Subset_2020/farm_report/{}'.format(report_farm_without_unc))
        plt.show()
        ...
        data = data.append(pd.DataFrame({'Farm Name': str(kf['p_name'].iloc[0]), '#Turbine': len(kf), 'Farm_lat':kf["ylat"].mean(), 'Farm_lon': kf["xlong"].mean(), 'AEP_No_Wake': power_rose.total_no_wake, 
                                         ...
                                         index=[0]), ignore_index=True)
        table_pickle = "Pickle_table_farms_Group_" + str(kf['p_name'].iloc[0]) + "_without_unc"
        data.to_pickle(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/Subset_2020/tabular_data_pickle/{}'.format(table_pickle))
        ...
        table_image = "Table_Image_" + str(kf['p_name'].iloc[0])+ "_without_unc"
        plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/Subset_2020/tabular_data_image/{}.png'.format(table_image))
