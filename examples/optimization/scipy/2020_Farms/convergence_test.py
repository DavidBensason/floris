...
# -*- coding: utf-8 -*-
"""
Created on Fri May 22 12:56:12 2020
...
if __name__ == '__main__':
    ...
    def render_mpl_table(table_new, col_width=2.8, row_height=0.625, font_size=14,
        ...
        return fig, ax
    data = pd.DataFrame([])
    maxiter_ls = [13,15,18,20,22] 
    for i in maxiter_ls:
        opt_options = {'maxiter': int(i), 'disp': False, 'iprint': 1, 'ftol': 1e-7, 'eps': 0.01} #Change
        ...
        kf = (mf.loc[mf['p_name'] == "Georgia Mountain"])
        ...
        #plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/100_Farms/farm_layout/{}'.format(layout_name))
        #ax.set_title('Baseline flow for U = 8 m/s, Wind Direction = 270$^\circ$')
        ...
        #plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/100_Farms/wind_rose/{}'.format(windrose_name))
        
        #Plot Ti rose 
        #wind_rose.plot_wind_rose_ti() 
        #ti_rose = str(kf['p_name'].iloc[0]) + "_ti_rose.png"
        #plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/100_Farms/ti_rose/{}'.format(ti_rose))
        
        #Plot Ti Winspeed dist.
        #wind_rose.plot_ti_ws() ## ALSO NOT WORKINg
        #ti_ws = str(kf['p_name'].iloc[0]) + "_ti_ws.png"
        #plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/100_Farms/ti_ws/{}'.format(ti_ws))
        ...
        if Optimization_case == "Unc_and_base":
            ...
            report_farm_without_unc = str(kf['p_name'].iloc[0]) + "_report_without_unc.png"
            ...
            df_turbine_power_opt['wd'] = df.wd
            # Summarize using the power rose module
            case_name_1 = 'Example '+str(kf['p_name'].iloc[0])+ 'Wind Farm with UNC'
            ...
            # Save farm report with designated name and path (this case  HPC)
            report_farm_with_unc = str(kf['p_name'].iloc[0]) + "_report_with_unc.png"
            ...
            data = data.append(pd.DataFrame({'Farm Name': str(kf['p_name'].iloc[0]), '#Turbine': len(kf), 'Farm_lat':kf["ylat"].mean(), 'Farm_lon': kf["xlong"].mean(), 'AEP_No_Wake': power_rose.total_no_wake, 
            ...
            table_pickle = "Pickle_table_farms_Group_" + str(kf['p_name'].iloc[0]) + "_without_unc"
            ...
            table_image = "Table_Image_" + str(kf['p_name'].iloc[0])+ "_without_unc"
            ...
            data1 = data1.append(pd.DataFrame({'Farm Name': str(kf['p_name'].iloc[0]), '#Turbine': len(kf), 'Farm_lat':kf["ylat"].mean(), 'Farm_lon': kf["xlong"].mean(), 'AEP_No_Wake': power_rose.total_no_wake, 
                                             'AEP_Baseline': power_rose.total_baseline, 'AEP_Opt':power_rose.total_opt, 
                                             '%_Baseline': 100.* power_rose.baseline_percent, '%_Opt': 100.* power_rose.opt_percent, 
                                             'Wk_Loss_Baseline':100.* power_rose.baseline_wake_loss, 'Wk_Loss_Opt': 100.* power_rose.opt_wake_loss, 
                                             'AEP_Gain_Opt': 100.* power_rose.percent_gain , 'Loss_Red_Opt':100.* power_rose.reduction_in_wake_loss}, 
                                             index=[0]), ignore_index=True)
            table_pickle_1 = "Pickle_table_farms_Group_" + str(kf['p_name'].iloc[0]) + "_with_unc"
            ...
            table_image_1 = "Table_Image_" + str(kf['p_name'].iloc[0])+ "_with_unc"
            ...
            df_turbine_power_opt['wd'] = df.wd
            # Summarize using the power rose module
            case_name = 'Example '+str(kf['p_name'].iloc[0])+ ' Wind Farm with UNC'
            power_rose = pr.PowerRose(case_name, df_power, df_turbine_power_no_wake, df_turbine_power_baseline,df_yaw, df_turbine_power_opt)
            ...
            data = data.append(pd.DataFrame({'Farm Name': str(kf['p_name'].iloc[0]), '#Turbine': len(kf), 'Farm_lat':kf["ylat"].mean(), 'Farm_lon': kf["xlong"].mean(), 'AEP_No_Wake': power_rose.total_no_wake, 
                                             ...
                                             'AEP_Gain_Opt': 100.* power_rose.percent_gain , 'Loss_Red_Opt':100.* power_rose.reduction_in_wake_loss}, 
                                             index=[0]), ignore_index=True)
            table_pickle = "Pickle_table_farms_Group_" + str(kf['p_name'].iloc[0]) + "_with_unc"
            data.to_pickle(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/tabular_data_pickle/{}'.format(table_pickle))
            ...
            table_image = "Table_Image_" + str(kf['p_name'].iloc[0])+ "_with_unc"
            plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/tabular_data_image/{}.png'.format(table_image))
            ...
            # Save farm report with designated name and path (this case  HPC)
            #report_farm_without_unc = str(kf['p_name'].iloc[0]) + "_report_without_unc.png"
            #plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/100_Farms/farm_report/{}'.format(report_farm_without_unc))
            plt.show()
            
            #Save final data as a pickle 
            data = data.append(pd.DataFrame({'Farm Name': str(kf['p_name'].iloc[0]), 'maxiter': float(i),'#Turbine': len(kf), 'Farm_lat':kf["ylat"].mean(), 'Farm_lon': kf["xlong"].mean(), 'AEP_No_Wake': power_rose.total_no_wake, 
                                             'AEP_Baseline': power_rose.total_baseline, 'AEP_Opt':power_rose.total_opt, 
                                             '%_Baseline': 100.* power_rose.baseline_percent, '%_Opt': 100.* power_rose.opt_percent, 
                                             'Wk_Loss_Baseline':100.* power_rose.baseline_wake_loss, 'Wk_Loss_Opt': 100.* power_rose.opt_wake_loss, 
                                             'AEP_Gain_Opt': 100.* power_rose.percent_gain , 'Loss_Red_Opt':100.* power_rose.reduction_in_wake_loss}, 
                                             index=[0]), ignore_index=True)
            #table_pickle = "Pickle_table_farms_Group_" + str(kf['p_name'].iloc[0]) + "_without_unc"
            #data.to_pickle(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/100_Farms/tabular_data_pickle/{}'.format(table_pickle))
            ...
            #table_image = "Table_Image_" + str(kf['p_name'].iloc[0])+ "_without_unc"
            #plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/100_Farms/tabular_data_image/{}.png'.format(table_image))
