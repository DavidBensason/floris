...
if __name__ == '__main__':
    ...
    for i in grid_spc:
        ...
        kf= "Patrick_test_D="+ str(d_lst[0]) +"_spc=" + str(i)
        N_turb = len(x)
        ...
        plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/Patduff_comp/{}'.format(layout_name))
        # ================================================================================
        ...
        if Optimization_case == "Unc_and_base":
            ...
            data = data.append(pd.DataFrame({'Farm Name': str(kf), '#Turbine': int(N_turb), 'Turbine_D':int(d_lst[0]),'Turb_spc_D': float(i), 'Farm_lat':wf_coordinate[0], 'Farm_lon': wf_coordinate[1], 'AEP_No_Wake': power_rose.total_no_wake, 
            ...
            data1 = data1.append(pd.DataFrame({'Farm Name': str(kf), '#Turbine': int(N_turb),'Turbine_D':int(d_lst[0]),'Turb_spc_D': float(i),'Farm_lat':wf_coordinate[0], 'Farm_lon': wf_coordinate[1], 'AEP_No_Wake': power_rose.total_no_wake, 
                                             'AEP_Baseline': power_rose.total_baseline, 'AEP_Opt':power_rose.total_opt, 
                                             '%_Baseline': 100.* power_rose.baseline_percent, '%_Opt': 100.* power_rose.opt_percent, 
                                             'Wk_Loss_Baseline':100.* power_rose.baseline_wake_loss, 'Wk_Loss_Opt': 100.* power_rose.opt_wake_loss, 
                                             'AEP_Gain_Opt': 100.* power_rose.percent_gain , 'Loss_Red_Opt':100.* power_rose.reduction_in_wake_loss}, 
            ...
            # Save farm report with designated name and path (this case  HPC)
            report_farm_without_unc = str(kf) +"_report_without_unc.png"
            plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/Patduff_comp/{}'.format(report_farm_without_unc))
            ...
            data.to_pickle(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/Patduff_comp/{}'.format(table_pickle))
            
            tabular = (data.loc[data['Turbine_spc_D'] == i])
            # Save final data as an image 
            ...
            table_image = "Table_Image_" + str(kf)+ "_without_unc"
            plt.savefig(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/Patduff_comp/{}.png'.format(table_image))
