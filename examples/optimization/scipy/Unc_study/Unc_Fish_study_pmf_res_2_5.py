...
if __name__ == '__main__':
    ...
    if constant_area_layout: ##THIS WORKS ATM 
        spc_N = (1852/spc) *(13/N_row)  #(1Nautical mile/164)
        spc_T= (1852/spc) * (8/T_row) #(1Nautical mile/164)
    ...
    for j in range(N_row):
        for k in range(T_row):
            layout_x.append(j*spc*D*math.cos(-45) - k*spc*D*math.cos(-45))
            layout_y.append(j*spc*D*math.cos(-45) + k*spc*D*math.cos(-45))
    ...
    unc_options={'std_wd': 4.95, 'std_yaw': 0.0,'pmf_res': pmf_res, 'pdf_cutoff': 0.95}
    # ================================================================================
    ...
    if calculate_new_wind_rose:
        ...
        df = wind_rose.load(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/wind_rose_pickle/Fishermans_Wind_Farm.p')
        #fi.floris.farm.flow_field.turbine_map.turbines.power_thrust_table["power"] = cp_8MW
    #fi.floris.farm.flow_field.turbine_map.turbines.power_thrust_table["thrust"] = ct_8MW
    #fi.floris.farm.flow_field.turbine_map.turbines.rotor_diameter = 164
    #f
    ...
    #windrose_name = str(kf1) + "_Wind_rose.png"
    ...
    if Optimization_case == "Unc_and_base":
        ...
        data = data.append(pd.DataFrame({'Farm Name': str(kf1), '#Turbine': int(Num_Turb),'pmf_res':float(pmf_res),'Turb_spc_D': int(spc),'Farm_lat':wf_coordinate[0], 'Farm_lon': wf_coordinate[1], 'AEP_No_Wake': power_rose.total_no_wake, 
        ...
        # Summarize using the power rose module
        case_name = 'Example '+str(kf1)+ ' Wind Farm without UNC'
        #power_rose = pr.PowerRose(case_name, df_power, df_turbine_power_no_wake, df_turbine_power_baseline,df_yaw, df_turbine_power_opt)
        power_rose = pr.PowerRose()
        power_rose.make_power_rose_from_user_data(
        	case_name,
        ...
        data = data.append(pd.DataFrame({'Farm Name': str(kf['p_name'].iloc[0]), '#Turbine': len(kf), 'Farm_lat':kf["ylat"].mean(), 'Farm_lon': kf["xlong"].mean(), 'AEP_No_Wake': power_rose.total_no_wake, 
                                         ...
                                         'AEP_Gain_Opt': 100.* power_rose.percent_gain , 'Loss_Red_Opt':100.* power_rose.reduction_in_wake_loss}, 
                                         index=[0]), ignore_index=True)
        table_pickle = "Pickle_" + str(kf1) + "_without_unc"
