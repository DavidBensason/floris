...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr  9 09:52:30 2020
...
if __name__ == '__main__':
    ...
    group= "TI_ThunderRanch_Study" ##Change for with next run 
    ...
    for i in T_i:
        ...
        kf1 = str(kf['p_name'].iloc[0]) + "_TI=" +str(i)
        ...
        if calculate_wind_rose:
            ...
            df = wind_rose.load(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/wind_rose_pickle/Thunder Ranch_Wind_Farm_no_TI.p')
        ...
        if Optimization_case == "Unc_and_base":
            ...
            #Save final data as a pickle 
            data = pd.DataFrame([])
            data = data.append(pd.DataFrame({'Farm Name': kf['p_name'].iloc[0], '#Turbine': len(kf),'TI':float(i), 'Farm_lat':kf["ylat"].mean(), 'Farm_lon': kf["xlong"].mean(), 'AEP_No_Wake': power_rose.total_no_wake, 
                                             'AEP_Baseline': power_rose.total_baseline, 'AEP_Opt':power_rose.total_opt, 
                                             '%_Baseline': 100.* power_rose.baseline_percent, '%_Opt': 100.* power_rose.opt_percent, 
                                             'Wk_Loss_Baseline':100.* power_rose.baseline_wake_loss, 'Wk_Loss_Opt': 100.* power_rose.opt_wake_loss, 
                                             'AEP_Gain_Opt': 100.* power_rose.percent_gain , 'Loss_Red_Opt':100.* power_rose.reduction_in_wake_loss}, 
