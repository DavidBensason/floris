...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr  8 14:34:30 2020
...
import six
"""
This is a study for seeing the effect of TI on the generic fishermans layout using a 1 nautical mile spacing. 
All files will be saved to a seperate folder in Saved_Fig called TI_Study.
"""
#import copy
#import floris.tools.wind_rose as rose
...
if __name__ == '__main__':
    ...
    data = pd.DataFrame([])
    data1 = pd.DataFrame([])
    group= "TI_study" ##Change for with next run 
    T_i = 0.12
    ...
    # Set the TI 
    fi.reinitialize_flow_field(turbulence_intensity=[T_i])
    kf= "TI_generic_study=" +str(T_i)
    #zf = "Fishermans"
    ...
    for j in range(N_row):
        for k in range(T_row):
            layout_x.append(j*spc_N*D*math.cos(-45) - k*spc_N*D*math.cos(-45))
            layout_y.append(j*spc_N*D*math.cos(-45) + k*spc_N*D*math.cos(-45))
    #remove option
    ...
    for count, turbine in enumerate(fi.floris.farm.flow_field.turbine_map.turbines):
            ...
            turbine.power_thrust_table["power"] = cp_8MW
            turbine.power_thrust_table["thrust"] = ct_8MW
    ...
    if calculate_new_wind_rose:
        ...
        df = wind_rose.load(r'/home/dbensaso/code/floris/examples/optimization/scipy/Saved_Fig/wind_rose_pickle/TI_study_wr_Wind_Farm.p')
    ...
    if Optimization_case == "Unc_and_base":
        ...
        data = data.append(pd.DataFrame({'Farm Name': str(kf), '#Turbine': int(Num_Turb), 'Turbine_D':int(T_i),'Turb_spc_D': int(spc_N), 'Farm_lat':wf_coordinate[0], 'Farm_lon': wf_coordinate[1], 'AEP_No_Wake': power_rose.total_no_wake, 
        ...
        data1 = data1.append(pd.DataFrame({'Farm Name': str(kf), '#Turbine': int(Num_Turb),'Turbine_D':int(T_i),'Turb_spc_D': int(spc_N),'Farm_lat':wf_coordinate[0], 'Farm_lon': wf_coordinate[1], 'AEP_No_Wake': power_rose.total_no_wake, 
                                         'AEP_Baseline': power_rose.total_baseline, 'AEP_Opt':power_rose.total_opt, 
                                         '%_Baseline': 100.* power_rose.baseline_percent, '%_Opt': 100.* power_rose.opt_percent, 
                                         'Wk_Loss_Baseline':100.* power_rose.baseline_wake_loss, 'Wk_Loss_Opt': 100.* power_rose.opt_wake_loss, 
                                         'AEP_Gain_Opt': 100.* power_rose.percent_gain , 'Loss_Red_Opt':100.* power_rose.reduction_in_wake_loss}, 
        ...
        data = data.append(pd.DataFrame({'Farm Name': str(kf), '#Turbine': int(Num_Turb),'TI':float(T_i),'Farm_lat':wf_coordinate[0], 'Farm_lon': wf_coordinate[1], 'AEP_No_Wake': power_rose.total_no_wake, 
                                         'AEP_Baseline': power_rose.total_baseline, 'AEP_Opt':power_rose.total_opt, 
                                         '%_Baseline': 100.* power_rose.baseline_percent, '%_Opt': 100.* power_rose.opt_percent, 
                                         'Wk_Loss_Baseline':100.* power_rose.baseline_wake_loss, 'Wk_Loss_Opt': 100.* power_rose.opt_wake_loss, 
                                         'AEP_Gain_Opt': 100.* power_rose.percent_gain , 'Loss_Red_Opt':100.* power_rose.reduction_in_wake_loss}, 
