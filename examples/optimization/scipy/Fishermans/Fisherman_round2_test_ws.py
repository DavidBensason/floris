...
# -*- coding: utf-8 -*-
"""
Created on Fri May 29 14:53:38 2020
...
## CASE NUMBER 2: Changing average wind speed with constant D, SP and spacing 
...
for i in avg_ws_list: 
    ...
    if scale_ws_avg:
        kf= "Fbase_spc="+ str(abs_spacing) +"_D=" + str(D) + '_avg_ws=' +str(des_avg_ws)\
        + '_SP=' + str(SP)
    else: 
        kf= "Fbase_spc="+ str(abs_spacing) +"_D=" + str(D) + \
        + '_SP=' + str(SP)
    ...
    for j in range(N_row):
        for k in range(T_row):
            layout_x.append(j*spc_N*math.cos(-45) - k*spc_N*math.cos(-45))
            layout_y.append(j*spc_N*math.cos(-45) + k*spc_N*math.cos(-45))
    #remove option
    ...
    if calculate_new_wind_rose:
        ...
        df = wind_rose.load(r'/home/dbensaso/floris/examples/optimization/scipy/Saved_Fig/wind_rose_pickle/Fishermans_no_TI_Wind_Farm.p')
    ...
    if scale_ws_avg: 
       ...
       # ws_list = np.arange(0,26,1)
        ...
        for ii in ws_list: 
            ...
            product = ii * freq_sum
            data_overall = data_overall.append({'ws': ii, 'freq': freq_sum, 'product': product}, ignore_index=True)
    ...
    if Optimization_case == "Unc_and_base":
        ...
        if scale_ws_avg:
            data = data.append(pd.DataFrame({'Farm Name': str(kf),'Rated Power': P_r, 'Specific Power':SP, '#Turbine': int(Num_Turb),'Turbine_D':int(D),
                                             ...
                                             'Turb_spc': float(abs_spacing),'Farm_lat':wf_coordinate[0], 'Farm_lon': wf_coordinate[1], 
        ...
        data.to_pickle(r'/home/dbensaso/floris/examples/optimization/scipy/Saved_Fig/Fishermans_base/table_pickle/{}'.format(table_pickle))
