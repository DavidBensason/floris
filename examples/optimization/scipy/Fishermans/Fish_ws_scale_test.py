...
# -*- coding: utf-8 -*-
"""
Created on Thu May 28 14:03:28 2020
...
data = pd.DataFrame([]) 
y_n = 5    
des_avg_ws_lis = [3,5,7,9,11,13,15,17]
for i in des_avg_ws_lis:
    
    scale_ws_avg = True
    if scale_ws_avg:
        des_avg_ws = i
    
    #D_lis = [100, 120, 140, 160, 180, 200]
    #for i in D_lis: 
    ...
    if scale_ws_avg:
        kf= "Fishermans_base_spc=" + '_avg_ws=' +str(des_avg_ws)
    #else: 
    #    kf= "Fishermans_base_spc="+ str(y_n) +"_D=" + str(i)
    ...
    if Optimization_case == "Unc_and_base":
        ...
        #Save final data as a pickle 
        if scale_ws_avg:
            data = data.append(pd.DataFrame({'Farm Name': str(kf),'Rated Power': P_r, '#Turbine': int(Num_Turb),'Turbine_D':int(D),
                                             'Avg_ws': float(des_avg_ws),
                                             'Turb_spc_D': int(y_n),'Farm_lat':wf_coordinate[0], 'Farm_lon': wf_coordinate[1], 
...
x = data['Avg_ws']
y = data['AEP_Baseline']
plt.xlabel('Avg_ws', fontsize=10)
plt.ylabel('AEP_Baseline', fontsize=10)
plt.plot(x,y, '-o')