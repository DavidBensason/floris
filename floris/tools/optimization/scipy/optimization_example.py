...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 15 15:27:13 2020
...
class YawOptimizationWindRose_sub(Optimization):
    """
    Sub class of the :py:class`floris.tools.optimization.Optimization`
    object class that performs yaw optimization for multiple wind speed,
    wind direction, turbulence intensity (optional) combinations.
    ...
    def optimize(self):
        ...
        for count, turbine in enumerate(self.fi.floris.farm.flow_field.turbine_map.turbines):
        ...
        for i in range(len(self.wd)):
            ...
            for j in range(len(cc)): #through each sub farm 
                ...
                for count, turbine in enumerate(self.fi.floris.farm.flow_field.turbine_map.turbines):
                    ...
                    turbine.power_thrust_table["thrust"] = ct_new
                """   
                #self.fi.reinitialize_flow_field(layout_array=(sub_farm_xcoord, sub_farm_ycoord), wind_direction=[270.0],wind_speed=[8.0])                
                #self.fi.calculate_wake()
                #pdb.set_trace()
                turbine_map = TurbineMap(sub_farm_xcoord, sub_farm_ycoord,[copy.deepcopy(self.fi.floris.farm.turbines[cc[j][ii]]) for ii in range(len(sub_farm_xcoord))])
                #turbine_map = TurbineMap(sub_farm_xcoord, sub_farm_ycoord,[copy.deepcopy(self.fi.floris.farm.turbines[ii]) for ii in range(len(sub_farm_xcoord))])

                #Sturbine_map = TurbineMap(sub_farm_xcoord, sub_farm_ycoord,[copy.deepcopy(self.fi.floris.farm.turbines[cc[j][ii]]) for ii in range(len(sub_farm_xcoord))])
                #Try wake stuff
                #self.fi.reinitialize_flow_field(layout_array=(sub_farm_xcoord, sub_farm_ycoord))
                #self.fi.calculate_wake()
                ...
                #self.fi_sub.reinitialize_flow_field(layout_array=(sub_farm_xcoord, sub_farm_ycoord))
                #pdb.set_trace()
                """
                # TEst turbine itteration 
                cp_8MW= [0,0,0,0,0.13,0.3,0.37,0.39,0.41,0.42,0.43,0.43,0.44,0.44,0.44,0.44,0.44,0.43,0.42,0.39,0.35,
                         0.31,0.28,0.25,0.23,0.2,0.18,0.17,0.15,0.14,0.13,0.12,0.11,0.1,0.09,0.08,0.08,0.07,0.07,0.06,
                         0.06,0.05,0.05,0.05,0.04,0.04,0.04,0.0]
                ...
                ct_8MW= [1.18,1.1,1.03,0.97,0.92,0.88,0.85,0.83,0.82,0.81,0.8,0.79,0.78,0.77,0.76,0.75,0.73,0.71,0.67,
                         0.6,0.52,0.45,0.39,0.34,0.3,0.27,0.24,0.22,0.19,0.18,0.16,0.15,0.14,0.13,0.12,0.11,0.1,0.09,
                         0.09,0.08,0.08,0.07,0.07,0.06,0.06,0.06,0.05,0.05]
                for count, turbine in enumerate(self.fi_sub.floris.farm.flow_field.turbine_map.turbines):
                    turbine.rotor_diameter = 109
                    turbine.hub_height = 164
                    cp_new = cp_8MW
                    ct_new = ct_8MW
